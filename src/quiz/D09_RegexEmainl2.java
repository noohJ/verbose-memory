...
public class D09_RegexEmainl2 {
	
	public static void main(String[] args) {
		
		Pattern email_pattern = Pattern.compile("(\\w+)@(\\w+\\.\\w+(\\.\\w+)*)");
		Matcher email_matcher = email_pattern.matcher(emails);
		
		List<String> email_list = new ArrayList<>();
		Map<String, Integer> domain_cnt = new HashMap<>();
		
		while (email_matcher.find()) {
			String email = email_matcher.group(0);
			String newShawk_id = email_matcher.group(1);
			String company_domain = email_matcher.group(2);
			
			email_list.add(email);
			Integer cnt;
			if ((cnt = domain_cnt.putIfAbsent(company_domain, 1)) != null) {
				domain_cnt.put(company_domain,  cnt+1);
			}
			domain_cnt.putIfAbsent(company_domain, null);
			
			
		}
		System.out.println("총 이메일 개수: "+email_list.size()+"개");
		System.out.println(domain_cnt);
		List<Domain> domainCountList = new ArrayList<>();
		for (String domain : domain_cnt.keySet()) {
			domainCountList.add(new Domain(domain, domain_cnt.get(domain)));
		}
		
		Collections.sort(domainCountList);
		System.out.println(domainCountList);
		
		for (Domain domain: domainCountList) {
